...
public static class ArraysTester {
    ...
    private static void RotateListRight(List<int> data, int amount)
    {
        ...
        var shiftright = new List<int>();
        int looping = data.Count + 1;
        int start = looping - amount - 1;
        while (looping !=1)
        {
            if (start == data.Count)
            {
            ...
            shiftright.Add(data[start]);
            start +=1;
            looping -=1;
        }
        for (int i = 0; i < shiftright.Count ; i++)
        {
            data.Remove(i+1);   
        }
        for (int i = shiftright.Count -1; i > -1 ; i--)
        {
            data.Insert(0,shiftright[i]);
        }
    }
}